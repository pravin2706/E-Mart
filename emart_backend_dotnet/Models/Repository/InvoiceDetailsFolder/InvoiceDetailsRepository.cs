...
namespace Emart_final.Models.Repository.InvoiceDetailsFolder
{
    public class Invoice_detailsRepository : IInvoiceDetailsRepository
    {
        ...
        {
            // Fetch the list of products associated with the specified invoiceID
            var products = await context.Invoice_Details
                .Where(id => id.invoiceID == invoiceID)
                .Select(id => id.Product) // Assuming Invoice_Details has a navigation property to Product
                .ToListAsync(); // Use ToListAsync to return a list of products
